...
- `dependencies_instrumentation` <Type text="boolean" /> <MetaInfo text="optional" />
  - Whether Wrangler should collect and send npm package dependency metadata (package names and versions) when deploying or uploading a Worker version. Defaults to `true`.
